...
// =============================================================================
// Cliente GestaoPay — gateway PIX brasileiro
// Doc oficial: https://gestaopay.readme.io/reference/introducao
//
...
const DEFAULT_BASE_URL = "https://api.gestaopayments.com";

export type GestaoPayConfig = {
  baseUrl: string;
  publicKey: string;
  secretKey: string;
};

function getConfig(): GestaoPayConfig {
  const publicKey = process.env.GESTAOPAY_PUBLIC_KEY;
  const secretKey = process.env.GESTAOPAY_SECRET_KEY;
  if (!publicKey || !secretKey) {
    throw new Error(
      "GestaoPay nao configurada. Defina GESTAOPAY_PUBLIC_KEY e GESTAOPAY_SECRET_KEY no .env.local",
    );
  }
  return {
    baseUrl: process.env.GESTAOPAY_API_URL || DEFAULT_BASE_URL,
    publicKey,
    secretKey,
  };
}
...
/** GestaoPay espera Int32 em centavos. */
...
export async function criarCobrancaPix(input: CriarPixInput): Promise<CriarPixResposta> {
  ...
  console.log("[gestaopay] POST", url, JSON.stringify(body));
  ...
  try {
    ...
    console.error("[gestaopay] resposta nao-JSON:", r.status, text);
    throw new Error(`GestaoPay resposta nao-JSON (status ${r.status}): ${text.slice(0, 500)}`);
  }

  if (!r.ok) {
    console.error("[gestaopay] erro:", r.status, JSON.stringify(data));
    const err = data as {
      message?: string;
      error?: string;
      errors?: unknown;
  ...
  if (!tx) {
    throw new Error(`GestaoPay resposta vazia/sem transacao: ${text.slice(0, 500)}`);
  }
  ...
  if (statusBruto === "REFUSED" || statusBruto === "ERROR") {
    throw new Error(
      `GestaoPay recusou (${statusBruto}): ${text.slice(0, 500)}`,
    );
  }

  if (!id || !qrcode) {
    throw new Error(
      `GestaoPay resposta sem id/qrcode (id=${id}, qrcode=${qrcode ? "ok" : "null"}): ${text.slice(0, 600)}`,
    );
  }
...
export async function consultarTransacaoPorId(id: string): Promise<ConsultarTransacaoResposta> {
  ...
  try {
    ...
    throw new Error(`GestaoPay resposta nao-JSON (status ${r.status}): ${text.slice(0, 300)}`);
  }
  if (!r.ok) {
    throw new Error(`GestaoPay consulta falhou (${r.status}): ${text.slice(0, 300)}`);
  }
...
/**
 * Mapeia o status da GestaoPay (uppercase) pro nosso enum interno.
