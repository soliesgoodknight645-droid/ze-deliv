...
import { mapearStatusPedido } from "@/lib/hyzepay";
import { broadcastStatusPedido } from "@/lib/realtime-pedido";
import { revalidateTag } from "next/cache";

// =============================================================================
// Webhook (postback) HyzePay
//
// Doc oficial: https://hyzepay.readme.io/reference/formato-dos-webhooks
//
...
function normalizarPayload(payload: unknown): Normalizado {
  ...
  // Pode vir flat (HyzePay) OU wrapped (alguns gateways usam {data: {...}}).
  const data =
  ...
  // ExternalId (HyzePay) === nosso pedido.numero (vai como external_ref nos items)
  const identifier =
    pegar<string>(
      data,
      "ExternalId",
  ...
  // HyzePay manda status em UPPERCASE. Mantemos uppercase pra mapear depois.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[webhook/hyzepay] body invalido", e);
  ...
  if (errUpd) {
    console.error("[webhook/hyzepay] update pedido falhou", errUpd);
    await sb
  ...
  await sb.from("eventos_pedido").insert({
    ...
    dados: {
      ...
      transactionId,
      descricao: isPago
        ? "Pagamento confirmado via PIX (HyzePay)"
        : `Webhook HyzePay ${evento}`,
    },
  });
...
export async function GET() {
  return NextResponse.json({ ok: true, webhook: "hyzepay", info: "use POST" });
}
